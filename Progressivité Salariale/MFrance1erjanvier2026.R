...
CALCULS_FR <- function(base, nombre_employes = 99) {
  ...
  #PRIME D'ACTIVITE
  # Constantes 2026
  # --- Paramètres prime d'activité AVANT le 1er avril 2026 (régime 01/04/2025 → 31/03/2026) ---
  montant_forfaitaire <- 633.21      # personne seule (vs 638.28 depuis le 01/04/2026)
  seuil_bonification  <- 700.92      # plancher = 59 × SMIC horaire brut (11,88 €)
  bonification_max    <- 184.27      # max (vs 240.63 depuis le 01/04/2026)
  plafond_bonif       <- 1443  
  base <- base %>%
    mutate(
      # Bonification : rampe linéaire de seuil_bonification jusqu'à plafond_bonif
      bonification = ifelse(salaire_net >= seuil_bonification,
                            pmin((salaire_net - seuil_bonification) *
                                   (bonification_max / (plafond_bonif - seuil_bonification)),
                                 bonification_max),
                            0),
      # Prime d'activité (abattement 59,85 % inchangé depuis le 01/04/2025)
