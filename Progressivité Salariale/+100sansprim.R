CALCULS_FR_SANS_PRIME <- function(base, nombre_employes = 99) {
  ...
  cot_pat <- base %>%
    mutate(
      ...
      exonerations_pat       = ifelse(salaire_brut >= 3 * SMIC_BRUT_2026, 0,
                                      pmax((ifelse(nombre_employes < 50, 0.3781, 0.3821)) *
                                             (0.5 * (3 * SMIC_BRUT_2026 / salaire_brut - 1))^1.75 + 0.02, 0) * salaire_brut)
    ) %>%
    mutate(
  ...
  # ── SEUL CHANGEMENT : prime d'activité forcée à 0 ──────────────────────
  base <- base %>%
    mutate(
      prime_activite                   = 0,
      salaire_net_impot_prime_activite = salaire_net_impot
    )
  ...
  base <- base %>% mutate(
    ...
    `taux d'IR (%net)`                               = impot_revenu_total / salaire_net,
...
# Sorties scénario sans prime d'activité ####
test_fr_sans_prime          <- CALCULS_FR_SANS_PRIME(base)
cotisations_fr_sans_prime   <- COIN_SOCIAL_FR(test_fr_sans_prime)
retraite_chomage_fr_sans_prime <- CHOMAGE_RETRAITE_FR(cotisations_fr_sans_prime)

final_FR_sans_prime <- retraite_chomage_fr_sans_prime %>%
  mutate(
    tx_cotis_chomage_FR  = cotisations_fr_sans_prime$assurance_chomage_pat / cotisations_fr_sans_prime$salaire_brut,
    transfo_chom_FR      = retraite_chomage_fr_sans_prime$taux_remplacement_chomage / tx_cotis_chomage_FR,
    tx_cotis_retraite_FR = (cotisations_fr_sans_prime$retraite_sal + cotisations_fr_sans_prime$retraite_pat) / cotisations_fr_sans_prime$salaire_brut,
    transfo_retraite_FR  = retraite_chomage_fr_sans_prime$taux_remplacement_retraite / tx_cotis_retraite_FR
  )